// GET → bridge /camera/status. Cheap JSON for polling whether the
// camera is up + how stale the latest frame is.
...
export const GET: APIRoute = async () => {
  ...
  return await new Promise<Response>((resolve) => {
    const req = httpRequest(
      { socketPath: SOCK, path: "/camera/status", method: "GET", timeout: 2000 },
      (res) => {
        const chunks: Buffer[] = [];
